...
function App() {
  ...
  return <div className={'app '+(['landing','signup'].includes(view) ? 'auth-app' : '')}>
    ...
    <main>
      ...
      {view === 'review' && <Review form={form} onBack={() => setView('financials')} onGenerate={() => { setView('generating'); setTimeout(() => setView('editor'), 900); }} />}
...
function Financials({setView}) {
  ...
  const expenseProjection=useMemo(()=>calculateOperatingExpenses(expenses,36,revenueMonthly),[expenses,revenueMonthly]);
  const operatingExpenses=expenseProjection.monthly[0]||0;
  ...
  return <div className="page financial-input-page"><div className="welcome"><div><span className="eyebrow">FINANCIAL INPUTS · 36-MONTH FORECAST</span><h1>Build your financial assumptions</h1><p>Structured inputs stay separate from your narrative questionnaire and update the forecast instantly.</p></div></div>
    <div className="finance-workspace"><nav className="finance-section-nav">{sections.map(([id,letter,title,caption])=><button key={id} className={active===id?'active':''} onClick={()=>setActive(id)}><span>{letter}</span><div><strong>{title}</strong><small>{caption}</small></div><ArrowRight size={15}/></button>)}</nav>
      <section className="finance-main"><div className="card financial-form"><div className="financial-form-head"><div><span className="eyebrow">{sections.find(x=>x[0]===active)[1]} · ASSUMPTIONS</span><h2>{sections.find(x=>x[0]===active)[2]}</h2><p>{sections.find(x=>x[0]===active)[3]}</p></div><span className="saved"><Check size={14}/>Saved automatically</span></div>
        ...
        {active==='expenses'&&<OperatingExpenses expenses={expenses} setExpenses={setExpenses} projection={expenseProjection} revenueMonthly={revenueMonthly} onPrevious={()=>setActive('startup')}/>}
...
function OperatingExpenses({expenses,setExpenses,projection,revenueMonthly,onPrevious}) {
  const emptyExpense=()=>({id:`expense-${Date.now()}`,name:'',category:'other',amount:0,calculationType:'Fixed amount',frequency:'Monthly',startMonth:1,endMonth:36,annualIncrease:0,notes:''});
  ...
  const saveExpense=()=>{
    ...
    commit(current=>current.some(item=>item.id===editing.id) ? current.map(item=>item.id===editing.id?editing:item) : [...current,editing]);
    setEditing(null);
  };
  ...
  const fixedTotal=expenses.filter(x=>x.calculationType!=='Percent of revenue').reduce((sum,x)=>sum+calculateOperatingExpenses([x]).threeYear,0);
  const revenueBasedTotal=Math.max(0,projection.threeYear-fixedTotal);
  return <div className="operating-expenses">
    ...
    <div className="expense-table-wrap"><table className="expense-table expense-table-detailed">
      <thead><tr><th>Expense</th><th>Category</th><th>Calculation</th><th>Frequency</th><th>Start</th><th>End</th><th>Year 1</th><th>Year 2</th><th>Year 3</th><th>Actions</th></tr></thead>
      <tbody>{expenses.length===0?<tr><td colSpan="10"><div className="expense-empty"><CircleDollarSign size={27}/><h3>No operating expenses have been added.</h3><p>Add recurring or revenue-based expenses such as rent, insurance, software, marketing, utilities, and professional fees.</p><button className="primary" onClick={openNew}><Plus size={16}/>Add First Expense</button></div></td></tr>:expenses.map(expense=>{const item=calculateOperatingExpenses([expense],36,revenueMonthly); return <tr key={expense.id}>
        <td><strong>{expense.name}</strong>{expense.notes&&<small>{expense.notes}</small>}</td><td>{expenseCategoryLabel(expense.category)}</td><td>{expense.calculationType==='Percent of revenue'?`${expense.amount}% of revenue`:`${money(Number(expense.amount||0))} fixed`}</td><td>{expense.frequency}</td><td>Month {expense.startMonth}</td><td>Month {expense.endMonth}</td><td>{money(item.monthly.slice(0,12).reduce((a,b)=>a+b,0))}</td><td>{money(item.monthly.slice(12,24).reduce((a,b)=>a+b,0))}</td><td>{money(item.monthly.slice(24,36).reduce((a,b)=>a+b,0))}</td><td><div className="expense-actions"><button title="Edit" aria-label={`Edit ${expense.name}`} onClick={()=>openEdit(expense)}><PencilLine size={15}/></button><button title="Duplicate" aria-label={`Duplicate ${expense.name}`} onClick={()=>duplicateExpense(expense)}><Copy size={15}/></button><button title="Delete" aria-label={`Delete ${expense.name}`} onClick={()=>removeExpense(expense.id)}><Trash2 size={15}/></button></div></td>
      </tr>})}</tbody>
    </table></div>
    <section className="expense-summary" aria-label="Operating expense summary">{totals.map((total,index)=><div key={index}><span>Year {index+1} operating expenses</span><strong>{money(total)}</strong></div>)}<div><span>Fixed expenses total</span><strong>{money(fixedTotal)}</strong></div><div><span>Revenue-based expenses total</span><strong>{money(revenueBasedTotal)}</strong></div><div><span>Number of active expenses</span><strong>{expenses.length}</strong></div></section>
    <section className="monthly-preview"><div><strong>Monthly preview</strong><span>First 12 months</span></div><div className="month-strip">{projection.monthly.slice(0,12).map((value,index)=><div key={index}><span>M{index+1}</span><b>{money(value)}</b></div>)}</div></section>
    ...
    {editing&&<div className="expense-modal-backdrop" role="presentation" onMouseDown={event=>event.target===event.currentTarget&&setEditing(null)}><section className="expense-modal" role="dialog" aria-modal="true" aria-labelledby="expense-form-title">
      ...
      <div className="expense-form-grid">
        <Field label="Expense name" value={editing.name} placeholder="e.g. Rent or insurance" onChange={value=>setEditing({...editing,name:value})}/><Select label="Category" value={editing.category} options={EXPENSE_CATEGORIES} onChange={value=>setEditing({...editing,category:value})}/>
        <Select label="Calculation" value={editing.calculationType||'Fixed amount'} options={['Fixed amount','Percent of revenue']} onChange={value=>setEditing({...editing,calculationType:value})}/><Field label={editing.calculationType==='Percent of revenue'?'Revenue percentage (%)':'Amount per payment ($)'} value={editing.amount} onChange={value=>setEditing({...editing,amount:value})}/>
        <Select label="Frequency" value={editing.frequency} options={['Monthly','Quarterly','Annually','One time']} onChange={value=>setEditing({...editing,frequency:value})}/><Field label="Start month (1–36)" value={editing.startMonth} onChange={value=>setEditing({...editing,startMonth:value})}/><Field label="End month (1–36)" value={editing.endMonth} onChange={value=>setEditing({...editing,endMonth:value})}/><Field label="Annual increase (%)" value={editing.annualIncrease} onChange={value=>setEditing({...editing,annualIncrease:value})}/><Field label="Notes (optional)" value={editing.notes} placeholder="Vendor or assumption" onChange={value=>setEditing({...editing,notes:value})}/>
      </div>{errors.length>0&&<div className="expense-errors" role="alert">{errors.map(error=><span key={`${error.field}-${error.message}`}>{error.message}</span>)}</div>}<div className="expense-modal-actions"><button className="secondary" onClick={()=>setEditing(null)}>Cancel</button><button className="primary" disabled={errors.length>0} onClick={saveExpense}>Save expense</button></div>
    </section></div>}
  </div>;
}
