...
export function getApprovedFinancialContext(snapshot:FinancialSnapshot,currentHash:string){
  ...
  return {snapshotVersion:snapshot.snapshotVersion,currency:snapshot.currency,annual:p.annual.slice(0,3).map((row,i)=>({year:row.projectionYear,revenue:row.revenue,grossProfit:row.grossProfit,grossMargin:row.grossMargin,ebitda:row.ebitda,netIncome:row.netIncome,endingCash:row.endingCash,endingDebt:row.endingDebt,debtService:row.debtService,dscr:a.annualMetrics[i]?.dscr.value??null})),funding:{totalProjectCost:p.totals.totalUses,ownerContribution:p.totals.totalOwnerContributions,proposedLoans:p.totals.totalLoanProceeds,otherFunding:ass.fundingSources?.filter(x=>!['owner_contribution','proposed_loan'].includes(x.type)).reduce((n,x)=>n+x.amount,0)||0},breakEven:{firstBreakEvenMonth:a.breakEven.firstOperatingBreakEvenMonth?.monthIndex??null,firstSustainedBreakEvenMonth:a.breakEven.firstSustainedBreakEvenMonth?.monthIndex??null,year1AverageBreakEvenRevenue:a.breakEven.yearOneAverageMonthlyBreakEvenRevenue},warnings:snapshot.warningsJson.filter(w=>w.severity==='warning').map(w=>({code:w.code,title:w.title,message:w.message}))};
}
