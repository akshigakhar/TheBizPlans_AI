...
const financialValue=(period:any,key:string)=>({
  ...
  endingDebt:Number(period?.balanceSheet?.currentPortionOfDebt||0)+Number(period?.balanceSheet?.longTermDebt||0),
}[key]??period?.[key]??0);
